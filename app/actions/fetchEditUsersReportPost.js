...
const fetchEditUsersReportPost = async (userName) => {
  const q = query(
    collection(db, "report-post"),
...
export const updatePost = async (postId, updatedData, file) => {
  ...
  if (file) {
    const storageRef = ref(
      storage,
      `report-posts/${randomFolderName}/${file.name}`
    );
...
export default fetchEditUsersReportPost;
